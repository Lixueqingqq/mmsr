...
def calc_ssim(sr, hr, scale, rgb_range, multichannel=True):
    if sr.size(-2) > hr.size(-2) or sr.size(-1) > hr.size(-1):
        print("the dimention of sr image is not equal to hr's! ")
        sr = sr[:,:,:hr.size(-2),:hr.size(-1)]
    img_num = sr.shape[0]
    ...
    for num in range(img_num):
        img1 = sr[num,:,:,:].clone()
        img1=img1.permute((1, 2, 0)).data.mul(255 /rgb_range).clamp(0, 255).cpu().numpy()
        img1=np.uint8(img1)
        img2 = hr[num,:,:,:].clone()
        img2=img2.permute((1, 2, 0)).data.mul(255 /rgb_range).clamp(0, 255).cpu().numpy()
        img2=np.uint8(img2)
        ssim_cal = ssim_cal + compare_ssim(img1, img2, multichannel=multichannel) 
    return ssim_cal/img_num
